...
recreate_env() {
    ...
    QUOTE_FILE="./.env-quote"
    ENV_FILE="./.env"
    if [ "$(echo ${env_files_pattern})" != "${env_files_pattern}" ]
    then
        echo "# DO NOT MODIFY THIS FILE" > "${TEMPLATE_FILE}"
        chmod 600 "${TEMPLATE_FILE}"
        echo "" > "${TEMPLATE_FILE}"
        echo "# This file has been generated by ${tool_name}. Use ct-* commands" >> "${TEMPLATE_FILE}"
        echo "" >> "${TEMPLATE_FILE}"
        cat ${env_files_pattern} | perl -ape 's{"}{\\"}g; s{^(.*?)=(.*)$}{$1="$2"}' >> "${TEMPLATE_FILE}"
        touch "${QUOTE_FILE}"
        chmod 600 "${QUOTE_FILE}"
        touch "${ENV_FILE}"
        chmod 600 "${ENV_FILE}"
        "${basedir}/resolve-template-env-file.sh" "${TEMPLATE_FILE}" "${QUOTE_FILE}" "600"
        cat "${QUOTE_FILE}" | perl -ape 's{^(.*?)="(.*)"$}{$1=$2}; s{\\"}{"}g;' > "${ENV_FILE}"
    fi
    popd >/dev/null
}
